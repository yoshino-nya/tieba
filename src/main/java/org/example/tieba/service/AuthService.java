...
import org.example.tieba.util.JwtUtil;
...
public class AuthService {
    ...
    public AuthResponse login(LoginRequest request) {
        ...
        if(user == null) {
            throw new BusinessException(ErrorCodeConstants.INVALID_USER_CREDENTIALS, "用户名或密码错误");
        }
    ...
    public AuthResponse register(RegisterRequest request) {
        if(userMapper.existsByUsername(request.getUsername())) {
            throw new BusinessException(ErrorCodeConstants.RESOURCE_ALREADY_EXISTS, "用户名已被使用");
        }
        User user = new User();
