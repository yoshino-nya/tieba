...
public class CommentService {
    ...
    public CommentResponse create(CreateCommentRequest req, Long postId) {
        ...
        if (req.getParentId() != null) {
            ...
            if (parentComment == null)
                throw new BusinessException(ErrorCodeConstants.NOT_FOUND, "要回复的评论不存在");

            // 回复的评论与当前评论不属于同一帖子，按理不该出现这种情况
            if (!Objects.equals(parentComment.getPostId(), postId))
                throw new BusinessException(ErrorCodeConstants.BAD_REQUEST, "无效的回复");
    ...
    public void delete(Long id) {
        ...
        if (userId == null)
            throw new BusinessException(ErrorCodeConstants.NOT_FOUND, "评论不存在或已被删除");
        if (!Objects.equals(userId, securityUtil.getCurrentUserId()))
            throw new BusinessException(ErrorCodeConstants.BAD_REQUEST, "无权限删除此评论");
